...
export interface AppleMusicAlbum {
  albumId: string;
  artwork?: { height: number; url: string; width: number };
  artistIds: string[];
  artistName: string;
...
export class AppleMusicProvider implements DiscoveryProvider {
  ...
  constructor(
    ...
    private readonly requestScope = "production",
  ) {}
  ...
  async scan(context: ScanContext): Promise<ProviderScanResult> {
    ...
    for (const [position, mapping] of selected.entries()) {
      if (context.onUnitStart) {
        const proceed = await context.onUnitStart({
      ...
      const identityScope = `${this.requestScope}:${mapping.canonicalArtistId}:${context.filter.since ?? "recent"}`;
      const viewPages: AppleMusicArtistViewPage[] = [];
      const missingViewErrors: AppleMusicClientError[] = [];
      for (const view of ["singles", "full-albums"] as const) {
        try {
          viewPages.push(
            await this.client.getArtistViewFirstPage(
              mapping.appleArtistId,
              view,
              context.signal,
              identityScope,
            ),
          ...
          if (isMissingAppleMusicResource(error)) {
            missingViewErrors.push(error);
            viewPages.push({ items: [], nextPresent: false });
            continue;
          }
          throw error;
        }
      }
      if (missingViewErrors.length === viewPages.length) {
        ...
        if (!artist) {
          const missingArtistError = missingViewErrors[0];
          if (!missingArtistError) throw new Error("Apple Music artist verification failed.");
          throw missingArtistError;
        }
      }
      const [singles, albums] = viewPages;
      if (!singles || !albums) throw new Error("Apple Music discovery views were not resolved.");
      const releases = dedupeBy([...singles.items, ...albums.items], (album) => album.albumId)
        .filter(
          (album) => !context.filter.since || (album.releaseDate ?? "") >= context.filter.since,
        )
        .sort(compareAlbumDate);
      const artistCandidates: TrackCandidate[] = [];
      const skippedReleaseIds = new Set<string>();
      for (const album of releases) {
        let songs: AppleMusicSong[];
        try {
          songs = await this.client.getAlbumTracks(album.albumId, context.signal, identityScope);
        } catch (error) {
          if (isSkippableAppleMusicRecordError(error)) {
            skippedReleaseIds.add(album.albumId);
            continue;
          }
          throw error;
        }
      ...
      await context.onBatch?.({
        ...
        currentUnit: mapping.canonicalName,
        currentUnitId: mapping.canonicalArtistId,
        partial: singles.nextPresent || albums.nextPresent,
        releases: releases.map((album) => ({
          backfillEligible: true,
          candidateCount: artistCandidates.filter(
            (candidate) => candidate.externalReleaseId === album.albumId,
          ).length,
          externalReleaseId: album.albumId,
          reasons: skippedReleaseIds.has(album.albumId)
            ? ["Apple Music release tracks were unavailable or invalid and were skipped"]
            : ["Apple Music shallow catalog view is within the scan window"],
          releaseDate: normalizeReleaseDate(album.releaseDate).date,
          releaseDatePrecision: normalizeReleaseDate(album.releaseDate).precision,
          releaseType: classifyAlbum(album),
          selectedForDetails: !skippedReleaseIds.has(album.albumId),
          title: album.title,
          totalTracks: album.trackCount ?? 0,
        })),
        stage: "recent_releases",
        totalUnits: selected.length,
      });
    }
    return { candidates };
...
function isMissingAppleMusicResource(error: unknown): error is AppleMusicClientError {
  return error instanceof AppleMusicClientError && error.status === 404;
}

function isSkippableAppleMusicRecordError(error: unknown): error is AppleMusicClientError {
  return (
    error instanceof AppleMusicClientError &&
    (error.status === 400 || error.status === 404 || error.classification === "invalid_payload")
  );
}
...
function normalizeAlbum(
  ...
  return {
    albumId: resource.id,
    ...(resource.attributes.artwork
      ? {
          artwork: {
            height: resource.attributes.artwork.height,
            url: resource.attributes.artwork.url,
            width: resource.attributes.artwork.width,
          },
        }
      : {}),
    ...
    evidenceUrl: `https://music.apple.com/${storefront}/album/${resource.id}`,
...
function toTrackCandidate(
  ...
  return {
    ...
    artistName: mapping.canonicalName,
    availability: "unavailable",
    credits: [{ name: mapping.canonicalName, role: "primary" }],
    ...(song.discNumber === undefined ? {} : { discNumber: song.discNumber }),
    ...
    title: song.title,
    ...(artwork ? { appleMusicRelease: artwork } : {}),
    ...(song.trackNumber === undefined ? {} : { trackNumber: song.trackNumber }),
    ...(album.upc ? { upc: album.upc } : {}),
  };
}
...
function appleMusicArtwork(
  album: AppleMusicAlbum,
  observedAt: Date,
): AppleMusicReleaseArtwork | undefined {
  if (!album.artwork || !album.evidenceUrl) return undefined;
  const imageUrl = normalizeAppleMusicArtworkUrl(
    album.artwork.url,
    album.artwork.width,
    album.artwork.height,
  );
  if (!imageUrl) return undefined;
  return {
    albumId: album.albumId,
    albumUrl: album.evidenceUrl,
    image: {
