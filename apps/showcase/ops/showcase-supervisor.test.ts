...
import {
  ...
  parsePid,
  restartDelayMs,
} from "./showcase-supervisor";

describe("Showcase supervisor", () => {
  ...
  it("removes dead and invalid PID records without deleting a live record", () => {
    const directory = mkdtempSync(join(tmpdir(), "showcase-supervisor-"));
    const pidPath = join(directory, "showcase.pid");
  ...
  it("claims one supervisor and safely replaces a dead supervisor record", () => {
    const directory = mkdtempSync(join(tmpdir(), "showcase-supervisor-lock-"));
    const pidPath = join(directory, "showcase-supervisor.pid");
  ...
  it("registers a direct, hidden, non-overlapping at-logon task with recovery", () => {
    const registration = readFileSync(
      resolve("scripts", "register-showcase-startup-task.ps1"),
      "utf8",
    );
    const removal = readFileSync(resolve("scripts", "remove-showcase-startup-task.ps1"), "utf8");

    expect(registration).toContain("Showcase Public Site Web Application");
    expect(registration).toContain("System32\\conhost.exe");
    expect(registration).toContain('--headless `"$node`" --import tsx');
    expect(registration).toContain("showcase-supervisor-cli.ts");
    ...
    expect(registration).toContain("-RestartCount 999");
    expect(registration).toContain("-Hidden");
    expect(registration).toContain("Start-ScheduledTask");
    expect(registration).toContain("showcase.stop");
    expect(registration).toContain("Refusing to stop unexpected process");
    ...
    expect(removal).toContain("showcase.stop");
    expect(removal).toContain("Refusing to stop unexpected process");
  });
});
