...
describe("one-time wake validation task registration", () => {
  it("is a direct hidden read-only task isolated from production workflows", () => {
    ...
    expect(registration).toContain("[Parameter(Mandatory = $true)]");
    expect(registration).toContain("TS New Music Radar Maintenance Wake Validation");
    expect(registration).toContain("$RunAt.ToString('yyyy-MM-dd-HHmm')");
