...
- `apps/showcase` remains a separate Next.js application with no imports from `@radar/db`, scanner,
  ...
  `apps/showcase/lib/generated-public-catalog.json`. The eight fictional release fixtures were
  removed. Six fixture artists remain for the unfinished editorial artist taxonomy.
...
- `showcase-public-v1` is generated by `apps/scanner/src/showcase-publish-cli.ts` through the root
  ...
  It never calls a provider. Detailed boundaries and the contract are in
  `docs/showcase-publication.md`.
- The current local export contains 371 real Apple-origin releases: 217 with confirmed Spotify album
  links and 154 with Apple Music only. It contains 368 released and 3 upcoming records. Four records
  have no persisted Apple track listing. Release genres remain empty and labels remain omitted
  because those values are not in normalized public-safe persisted fields. Neutral artwork
...
- Publication validation passed: lint, all seven workspace type checks, 69 unit files with 498
  tests, 28 database integration files with 149 tests against the isolated test service, the
  Showcase production build with 384 generated pages, and all five Showcase Playwright tests. The
  new tests prove Spotify-only exclusion, Apple-only inclusion, strict reconciled Spotify links,
  private-field non-copying, and no Showcase database or provider API runtime dependency. No live
  provider activity was used.
- The production worktree was observed on `codex/release-radar-hardening` at `89c06e6671...` with
  separate uncommitted discovery-maintenance work and `outputs/`. This Showcase task made no writes
  there. Its private `.env` was read only by the one-time local export process and was not copied or
  changed.
...
- Current observed HEAD: `89c06e6671be9d66a3f00e016e69c3619deba8dc`
- Unrelated discovery-maintenance edits and `outputs/` are present in that worktree. They were not
