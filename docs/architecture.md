...
The next scheduler milestone is designed in [spotify-rolling-scheduler-design.md](spotify-rolling-scheduler-design.md). It replaces burst-style automatic operation with short-lived periodic ticks, one durable PostgreSQL work queue, dynamic 24-hour due times, a future 10-second global request-start minimum, and explicit planning, validation, automatic, paused, and disabled modes. That scheduler is designed but not implemented, live-provider tested, or proven at scale. Existing manual batches remain available and Batch 3 remains untouched.
