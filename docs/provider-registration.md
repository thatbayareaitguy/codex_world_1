...
Keep `SPOTIFY_PLAYLIST_WRITES_ENABLED=false` and `SPOTIFY_ALLOWED_PLAYLIST_ID=` for initial authorization. Create the desired private playlist directly in Spotify before enabling the implemented add-only export. Set its 22-character ID in `SPOTIFY_ALLOWED_PLAYLIST_ID`, reconnect Spotify to grant `playlist-modify-private`, and then explicitly enable writes. The application has no playlist picker and cannot create or modify playlist properties. Every live export re-verifies that the configured playlist ID is the returned playlist, is owned by the connected account, and remains private and non-collaborative. Dry runs and live exports use the same canonical eligibility and ordering plan; live exports only add tracks and persist an application-owned ledger for restart safety and idempotency.
