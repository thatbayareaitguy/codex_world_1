...
Keep `SPOTIFY_PLAYLIST_WRITES_ENABLED=false` and `SPOTIFY_ALLOWED_PLAYLIST_ID=` for initial authorization. Create the desired private playlist directly in Spotify before enabling the implemented add-only export. Set its 22-character ID in `SPOTIFY_ALLOWED_PLAYLIST_ID`, enable writes, and reconnect Spotify with forced consent to grant both `playlist-modify-private` and `playlist-modify-public`. Requesting the public modification scope does not permit the UI or server to choose a public playlist or change visibility. The application has no playlist picker and cannot create or modify playlist properties. Every live export re-verifies that the configured playlist ID is the returned playlist, is owned by the connected account, and remains private and non-collaborative. Dry runs and live exports use the same canonical eligibility and ordering plan; live exports only add tracks and persist an application-owned ledger for restart safety and idempotency.

Use `pnpm spotify:playlist-export -- --dry-run` to inspect the configured target, additions, skips, and ordering without mutation. Use `pnpm spotify:playlist-export -- --live` only after the dry run is correct and the write scope is present. Neither command accepts a playlist ID argument.
