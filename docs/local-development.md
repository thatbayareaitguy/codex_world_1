...
Spotify playlist writes are off by default. Leave `SPOTIFY_PLAYLIST_WRITES_ENABLED=false` and `SPOTIFY_ALLOWED_PLAYLIST_ID=` for read-only authorization. For an explicitly approved add-only export, manually create a private non-collaborative playlist in Spotify, configure its 22-character ID, enable writes, and reconnect Spotify once to grant `playlist-modify-private`. The browser and CLI cannot select or override the configured ID.
