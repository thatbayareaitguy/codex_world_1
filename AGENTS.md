...
- Spotify membership writes are additions only to the single valid `SPOTIFY_ALLOWED_PLAYLIST_ID`, must pass enabled, target, ownership, private, and non-collaborative checks, and may use only exact or manually confirmed tracks. The internal Custom Order maintainer may reorder existing ranges only on that target with snapshot-aware `PUT /playlists/{id}/items`; it must never remove or re-add an item to order it.
- Never expose Spotify playlist creation, selection, rename, visibility changes, artwork upload, follow, unfollow, remove, replace, or arbitrary reorder operations. The browser and request bodies cannot choose a reorder target.
